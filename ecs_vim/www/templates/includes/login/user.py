...
@frappe.whitelist(allow_guest=True)
def sign_up(email, full_name,mobile, redirect_to):
    ...
    if user:
        ...
        session_otp_user = frappe.get_doc({
            "doctype": "Session OTP Users",
            "email": email,
            "first_name": escape_html(full_name),
            "enabled": 1,
            "new_password": random_string(10),
            "user_type": "Website User",
            "csrf_token": frappe.local.request_ip,
            "user":user.name,
            "phone_no": mobile
        })
        if not frappe.db.exists("Session OTP Users", {"csrf_token":frappe.local.request_ip, "user":user.name, "phone_no":mobile}):
            session_otp_user.insert(ignore_permissions=True)
        ...
        if user.flags.email_sent:
            return session_otp_user.name, _("Please check your email for verification")
        else:
            return session_otp_user.name, _("Please check your email for verification")
