...
import frappe
from ecs_vim.ecs_vim.doctype.vacation_rejoining.vacation_rejoining import (
    vacation_rejoining_edit,
)
...
def validate(doc, method):
    if not doc.rejoining_date:
        doc.vacation_opening_balance = 0
...
def validate_rejoining_date(doc):
    if doc.rejoining_date and getdate(str(doc.rejoining_date)) < getdate(
        str(doc.date_of_joining)
    ):
...
def fetch_child(templete, date_of_joining):
    ...
    for i in data:
        ...
        document = i["document"]
        list_data.append({"document": document, "date": dat})
