...
from ecs_vim.custom_methods import set_comp_val, get_comp_name, get_leve_name
from ecs_vim.ecs_vim.doctype.leave_request.leave_request import (
    get_leave_rule,
    compenstation_percent,
)
from ecs_vim import month_30days
...
def validate(doc, method):
    ...
    doc.net_pay = flt(doc.get("gross_pay")) - (
        flt(doc.get("total_deduction")) + flt(doc.get("total_loan_repayment"))
    )
...
def salary_component_value(doc, include_current=False):
    payroll_dict = get_payroll_period(
        ...
        actual_period=int(include_current),
    )
...
def get_payroll_period(company, start_date, end_date, actual_period=0):
    ...
    sel_payroll = """SELECT name, {3}
		...
		WHERE company = '{0}'
		AND start_date = '{1}'
		AND end_date = '{2}'""".format(
        company, start_date, end_date, field
    )
    ...
    if get_payroll:
        ...
        frappe.throw(
            _(
                "There is no Payroll Period Set for {} - {}".format(
...
def get_actual_structure(doc, payroll_dict):
    ...
    if struct and struct.get("salary_structure"):
        doc._actual_salary_structure_doc = frappe.get_doc(
            "Salary Structure", struct.get("salary_structure")
        )
    else:
    ...
    for key in ("earnings", "deductions"):
        for struct_row in doc._actual_salary_structure_doc.get(key):
            ...
            prev_sal_ded.setdefault(key, {}).setdefault(
                struct_row.get("salary_component"),
                {
...
def get_component_amt(doc, payroll_dict, prev_sal_ded, include_current):
    ...
    for leave_type in leave_dict.keys():
        ...
        leave_days += compensation_amt(
            prev_sal_ded.get("earnings", {}), compensation, payroll_dict, leave_type
        )
    ...
    for ab_dict in absent_dict.values():
        ...
        abs_days = compensation_amt(
            prev_sal_ded.get("earnings", {}),
            compensation,
            payroll_dict,
            ab_dict.get("leave_type") or "Absent",
        )

        # remove weakoff
        if ab_dict.get("leave_type"):
            abs_days = 0
...
def get_attendance(doc, payroll_dict):
    attendance_list = """SELECT status, leave_type, count(*) attendance, GROUP_CONCAT(DISTINCT leave_application) as application, GROUP_CONCAT(DISTINCT attendance_date) as dates
		...
		GROUP BY status, leave_type""".format(
        doc.employee, payroll_dict.get("start_date"), payroll_dict.get("end_date")
    )
    ...
    for row in attendance_list:
        ...
        # else:
        attendance_dict.setdefault(row.get("status"), {}).setdefault(
            row.get("leave_type") or row.get("status"), row
        )
...
def get_compansation(doc, leave, payroll_dict):
    ...
    for leave_appl in leave.get("application", "").split(","):
        ...
        if leave_appl:
            componsation = process_compansation(
                doc, leave_appl[0].get("leave_request"), payroll_dict
            )
            for leave_rule in componsation.keys():
                ...
                for k_com in comp_dict.keys():
                    compansation_dict[leave_rule][k_com] = (
                        compansation_dict[leave_rule].get(k_com, 0) + comp_dict[k_com]
                    )
...
def process_compansation(doc, leave_req, payroll_dict):
    ...
    for com in compansation:
        ...
        compansation_dict[com.get("leave_rule")][
            com.get("approved_compensation", 0)
        ] = (
            compansation_dict[com.get("leave_rule")].get(
                com.get("approved_compensation", 0), 0
            )
            + days
        )
    ...
    for leav_rule, row_comp in comp.items():
        ...
        for per, days in row_comp.items():
            ...
            for elem, row in earnings.items():
                ...
                if not row.get("compensation") or (
                    row.get("compensation")
                    and not row.get("compensation").get(leave_type)
                ):
                    row.setdefault("compensation", {}).setdefault(leave_type, comp_val)
                elif row.get("compensation") and row.get("compensation").get(
                    leave_type
                ):
                    row.get("compensation")[leave_type] = (
                        float(row.get("compensation")[leave_type] or 0.0) + comp_val
                    )
...
def check_sal_struct(doc, joining_date, relieving_date, payroll_dict):
	...
	"""
        % cond,
        {
            ...
            "start_date": payroll_dict.get("start_date"),
            "end_date": payroll_dict.get("end_date"),
...
def working_days(doc):
    ...
    if joining_date:
        if (
            getdate(doc.get("start_date"))
            <= joining_date
            <= getdate(doc.get("end_date"))
        ):
...
def get_ot_amt(doc, payroll_dict, prev_sal_ded, include_current):
    attendance_list = """SELECT `A`.`ot_rule`, `A`.`shift`
		...
		-- , `A`.`leave_type`
		HAVING SUM(`A`.`ot_hours`) > 0""".format(
        doc.employee, payroll_dict.get("start_date"), payroll_dict.get("end_date")
    )
    attendance_list = frappe.db.sql(attendance_list, as_dict=True)
    ...
    leave_rule = get_leave_rule(
        date=payroll_dict.get("end_date"), leave_type=leave_type
    )
    if not leave_rule:
        return
    ...
    for ot_row in attendance_list:
        ...
        override_hrs = frappe.db.get_value(
            "Shift Type", ot_row.get("shift", 1), "override_working_hours"
        )
        ...
        if ot_row.get("week_off") == get_leve_name("WO"):
            ot_amt += (
                ot_row.get("ot")
                * ot_rule_doc.get("non_working_day_ot_rate")
                * (component_val / daily_working_hrs)
            )
        elif ot_row.get("week_off") == get_leve_name("H"):
            ot_amt += (
                ot_row.get("ot")
                * ot_rule_doc.get("holiday_ot_rate")
                * (component_val / daily_working_hrs)
            )
        else:
            ot_amt += (
                ot_row.get("ot")
                * ot_rule_doc.get("working_day_ot_rate")
                * (component_val / daily_working_hrs)
            )
    ...
    if not ab_dict.get("leave_type"):
        for date in ab_dict.get("dates", "").split(","):
            ...
            for percent in compansation.keys():
                ...
                compansation_dict[leave_rule.get("name")][percent] = compansation_dict[
                    leave_rule.get("name")
                ].get(percent, 0) + len(compansation[percent])
...
def get_absent_for_period(employee, from_date, leave_rule):
    ...
    if from_date and leave_rule.get("leave_credit_frequency") == "Yearly":
        ...
        condition = " and attendance_date between '{}' and '{}'".format(
            year_start, year_end
        )
...
def late_coming_amt(doc, payroll_dict, prev_sal_ded, include_current):
    ...
    for st_row in attendance_list:
        ...
        override_hrs = frappe.db.get_value(
            "Shift Type", st_row.get("shift", 1), "override_working_hours"
        )
