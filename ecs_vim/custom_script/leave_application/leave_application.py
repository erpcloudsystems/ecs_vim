...
from ecs_vim.custom_script.common_code import raise_link_exists_exception
...
def validate(doc, method):
    if validate_ss_processed(doc.employee, doc.from_date) or validate_ss_processed(
        doc.employee, doc.to_date
    ):
        frappe.throw(
            "Can not create or edit leave application, Salary is already processed for this month"
        )
...
def validate_leave_overlap(doc):
    ...
    for d in frappe.db.sql(
		...
		and name != %(name)s""",
        {
            ...
            else doc.vacation_leave_application,
        },
        as_dict=1,
    ):
        if d:
...
def validate_half_day_leave(doc, d):
    if doc.doctype != "Vacation Leave Application":
        if (
            cint(doc.half_day) == cint(d.half_day)
            and getdate(doc.half_day_date) == getdate(d.half_day_date)
            and (
                flt(doc.total_leave_days) == 0.5
                and (
                    getdate(doc.from_date) == getdate(d.to_date)
                    or getdate(doc.to_date) == getdate(d.from_date)
                )
            )
        ):
...
def on_trash(doc, method):
    if (
        ...
        and doc.employee
        and doc.to_date
        and validate_ss_processed(doc.employee, doc.from_date)
        or validate_ss_processed(doc.employee, doc.to_date)
    ):
        frappe.throw(
            "Can not delete leave application, Salary is already processed for this month"
        )
    # cannot_cancel(doc)
...
def update_attendance_cancel(doc):
    attendance = frappe.db.sql(
		...
		and docstatus < 2""",
        (doc.employee, doc.from_date, doc.to_date),
        as_dict=1,
    )
    if attendance:
        for i in attendance:
    ...
    if doc.status == "Approved":
        ...
        if attendance:
            for d in attendance:
                ...
                if status == "Present":
                    frappe.msgprint(
                        "Attendance already marked as present", raise_exception=True
                    )
                    ...
                    if status == "Absent":
                        ...
                        status = "Half Day"
                frappe.db.sql(
                    """update `tabAttendance` set status = %s,status2 = %s, leave_type = %s\
						where name = %s""",
                    (status, doc.leave_type, doc.leave_type, d.name),
                )
...
def cannot_cancel(doc):
    ...
    if len(attendance) > 0 and doc.get("auto_cancel_from_attendance", 0) == 0:
        frappe.throw(
            "Cannot Delete or Cancel It is Linked With Attendance <a href='#Form/Attendance/{0}'>{0}</a>".format(
                attendance[0]["name"]
            )
        )
...
def attendace_delete(doc):
    attendance = frappe.db.sql(
		...
		and (attendance_date between %s and %s)""",
        (doc.employee, doc.name, doc.from_date, doc.to_date),
        as_dict=1,
    )
    if attendance:
...
def link_field(doc):
    doc_name = (
        "Leave Request" if doc.get("leave_request") else "Vacation Leave Application"
    )
...
def validate_salary_processed_days(doc, method):
    ...
    if last_processed_pay_slip:
        frappe.throw(
            _(
                "Salary already processed for period between {0} and {1}, You Cannot Change Leave Application between this date range."
            ).format(
...
def get_user_role(doc, method):
    ...
    if int(workflow_del or 0) == 1:
        ...
        if emp_doc.user_id:
            ...
            if (
                len(workflow) > 0
                and doc.doctype == "Leave Application"
                and doc.ignore_workflow_delegation == 0
                and doc.workflow_delegation_id == None
                and doc.status == "Open"
            ):
        ...
        if wf_doc.docstatus == 1:
            ...
            frappe.db.sql(
                "UPDATE `tabWorkflow Delegation` SET reference_id ='{0}' where name ='{1}' ".format(
                    wf_id, doc.workflow_delegation_id
                )
            )
        else:
            wf_doc.reference_id = ""
