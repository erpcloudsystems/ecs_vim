...
def create_customer(clist):
    ...
    try:
        log_doc = setLog(
            endpoint="ecs_vim.salesforce_api.create_customer", req_body=clist
        )
        for cust in clist["data"]:
            dup_doc = validate_duplicate("Customer", sfid=cust["salesForceID"])
            if dup_doc:
                res.append(
                    {"salesforceID": cust["salesForceID"], "ErpCode": dup_doc[0].name}
                )
                continue
            ...
            res.append(
                {"salesforceID": new_cust.salesforceid, "ErpCode": new_cust.name}
            )

        setLog(doc_name=log_doc, status="Success", response=res)
        return {"status": "success", "succeeded_records": res}
    except Exception as ex:
        setLog(doc_name=log_doc, status="Error", response=res, error=str(ex))
        return {"status": "error", "message": ex, "succeeded_records": res}
...
def create_contact(clist):
    ...
    try:
        log_doc = setLog(
            endpoint="ecs_vim.salesforce_api.create_contact", req_body=clist
        )
        for cust in clist["data"]:
            dup_doc = validate_duplicate(
                "Family", cust=cust["AccERPcode"], sfid=cust["SalesforceId"]
            )
            if dup_doc:
                res.append(
                    {
            ...
            fam_acc_res = frappe.db.get_list(
                "Customer Family Detail",
                filters={
                    "parent": cust["AccERPcode"],
                    "salesforceid": cust["SalesforceId"],
                },
            )
            res.append(
...
def validate_duplicate(type=None, sfid=None, cust=None):
    if type == "Customer":
        ...
        return frappe.db.get_list(
            "Customer Family Detail", filters={"parent": cust, "salesforceid": sfid}
        )


def setLog(
    endpoint=None, req_body=None, doc_name=None, status=None, response=None, error=None
):
    if doc_name:
        frappe.db.set_value(
