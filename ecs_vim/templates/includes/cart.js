...
$.extend(shopping_cart, {
  ...
  get_update_address_dialog() {
    let d = new frappe.ui.Dialog({
      ...
      primary_action: () => {
        ...
        const address_type = $card
          .closest("[data-address-type]")
          .attr("data-address-type");
        const address_name = $card
          .closest("[data-address-name]")
          ...
          callback: function (r) {
            ...
            if (!r.exc) {
              ...
              shopping_cart.parent
                .find(`.address-container[data-address-type="${address_type}"]`)
  ...
  bind_change_qty: function () {
    ...
    $(".cart-items").on("change", ".cart-qty", function () {
      ...
      var min = $(this).attr("min-val");
      var min = $(this).attr("max-val");
    });
  ...
  bind_coupon_code: function () {
    $(".bt-coupon").on("click", function () {
      //  var elts = document.getElementsByClassName('item-subtitle');
      // var items=[]
      // for (var i = 0; i < elts.length; ++i) {
      //     var cur_row={}
      //     cur_row.name=String(elts[i].innerHTML)
      //     items.push({ "name": (String(elts[i].innerHTML).replace(/(\r\n|\n|\r|\t)/gm, "")) } );

      // }
      //     frappe.call({
      //         method: "ecs_vim.custom_script.point_of_sale.point_of_sale.validate_coupen_item",
      //         args : {
      //             items:items,
      //             applied_code :  $('.txtcoupon').val().toUpperCase()
  ...
  apply_coupon_code: function (btn) {
    console.log("mesage", $(".txtcoupon").val());
    var elts = document.getElementsByClassName("item-subtitle");
    var items = [];
    for (var i = 0; i < elts.length; ++i) {
      var cur_row = {};
      cur_row.name = String(elts[i].innerHTML);
      items.push({
        name: String(elts[i].innerHTML).replace(/(\r\n|\n|\r|\t)/gm, ""),
      });
    }
    frappe.call({
      method:
        "ecs_vim.custom_script.point_of_sale.point_of_sale.validate_coupen_item",
      args: {
        items: items,
        applied_code: $(".txtcoupon").val().toUpperCase(),
      },
