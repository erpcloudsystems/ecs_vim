class ItemConfigure {
  ...
  show_configure_dialog() {
    ...
    this.dialog.set_values(
      JSON.parse(localStorage.getItem(this.get_cache_key()))
    );
    ...
    if (changed_value && changed_value.includes(" to ")) {
      ...
      if (!numbers.some((n) => isNaN(n))) {
        ...
        const control = frappe.ui.form.make_control({
          df: {
            fieldtype: "Int",
            label: __("Enter value betweeen {0} and {1}", [
              numbers[0],
              numbers[1],
            ]),
            change: () => {
  ...
  get_attributes_and_values() {
    return this.call(
      "erpnext.portal.product_configurator.utils.get_attributes_and_values",
      {
        item_code: this.item_code,
      }
    );
