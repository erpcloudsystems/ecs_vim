...
def add_data(company):
    ...
    master_data = json.loads(
        open(frappe.get_app_path("ecs_vim", "setup_wizard", "default_data.json")).read()
    )
    ...
    try:
        if master_data:
            ...
            for data in master_data:
                ...
                is_single_doc = frappe.db.get_value(
                    "DocType", data["doctype"], "issingle"
                )
                ...
                if is_single_doc != 1:
                    ...
                    if old_data:
                        for d in old_data:
                            ...
                            if (
                                del_doc.get("company")
                                and del_doc.get("company") != company
                            ):
                                continue
                            # frappe.errprint(del_doc.docstatus)
                            if (
                                del_doc.docstatus == 1
                                and data["doctype"] != "Benefit Type for EOS"
                            ):
                                del_doc.cancel()
                            # frappe.delete()
                ...
                for d in range(0, len(data["data"])):
                    ...
                    if data["doctype"] == "Benefit Type for EOS":
                        if frappe.db.exists(
                            {"doctype": data["doctype"], "name": doc1["name"]}
                        ):
                            continue
