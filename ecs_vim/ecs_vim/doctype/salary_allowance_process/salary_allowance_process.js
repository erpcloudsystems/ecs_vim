...
frappe.ui.form.on('Salary Allowance Process', {
	...
	get_template: function(frm) {
		window.location.href = repl(frappe.request.url +
			'?cmd=%(cmd)s&company=%(company)s&from_date=%(from_date)s&to_date=%(to_date)s', {
				cmd: "ecs_vim.ecs_vim.doctype.salary_allowance_process.salary_allowance_process.get_template",
	...
	excel_upload: function(frm) {
		...
		new frappe.ui.FileUploader({
			wrapper: $wrapper,
			method: 'ecs_vim.ecs_vim.doctype.salary_allowance_process.salary_allowance_process.excel_upload',
			// allow_multiple: 0,
