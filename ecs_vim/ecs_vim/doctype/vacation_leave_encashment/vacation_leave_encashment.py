...
class VacationLeaveEncashment(Document):
    def validate(self):
        ...
        if flt(self.amount or 0) <= 0:
            frappe.throw(
                "Vacation Leave Encashment Cannot be Created for value Less then or Equal to Zero"
            )
    ...
    def valid_from_date(self, cancel=None):
        ...
        if res_vac_leav_encash:
            ...
            if res_vac_leav_encash["docstatus"] == 0 and not cancel:
                frappe.throw(
                    'Vacation Leave Encashment Is Not Approved For Application <a href="#Form/Vacation Leave Encashment/{0}">{0}</a>'.format(
                        res_vac_leav_encash["name"]
                    )
                )
    ...
    def calcuate_days(self, set_pay=1):
        self.days = calculate_vacation(self.employee, self.to_date, self.name, 1)[
            "eligible_days"
        ]
        if set_pay:
    ...
    def calcuate_days_and_amount(self):
        try:
            ...
            gross_ear = vacation_encash_amount(
                earnings, (self.pay_days if self.pay_days > 0 else 0)
            )

            deductions = earning_deduction.get("deductions", {})
            gross_ded = vacation_encash_amount(
                deductions, (self.pay_days if self.pay_days > 0 else 0)
            )
    ...
    def make_journal_entry(self):
        ...
        journal_entry.user_remark = _("Journal Entry of {0} from {1} to {2}").format(
            self.doctype, self.from_date, self.to_date
        )
    ...
    def get_company_series(self):
        series = frappe.db.get_value(
            "Company", {"company_name": self.company}, "series"
        )

        if not series:
            frappe.throw(_("Please set Series in Company {0}").format(self.company))
...
def vacation_encash_amount(component_dict, payable_days):
    closing_comp = frappe.db.sql_list(
        "SELECT name FROM `tabSalary Component` WHERE vacation_closing = 1"
    )
    total_vacation_amt = 0
    for comp, row in component_dict.items():
        if comp in closing_comp:
            row["vacation_amount"] = (
                row.get("default_amount", 0) / (360 / 12)
            ) * payable_days
        else:
