...
frappe.ui.form.on("Leave Request", {
	...
	onload: function(frm) {
		...
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method: "ecs_vim.ecs_vim.doctype.leave_request.leave_request.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.employee) {
			frappe.call({
				method: "ecs_vim.ecs_vim.doctype.leave_request.leave_request.get_leave_details",
				async: false,
				args: {
					employee: frm.doc.employee,
    ...
    make_workflow_delegation: function(frm){
		frappe.call({
			method: "ecs_vim.ecs_vim.doctype.leave_request.leave_request.make_workflow_delegation",
			// doc: frm.doc,
            args: {
	...
	get_workflow_delegation:function(frm){
        if(frm.doc.employee && frm.doc.from_date && frm.doc.to_date){
            frappe.call({
                method:'ecs_vim.ecs_vim.doctype.leave_request.leave_request.get_workflow_delegation',
                args:{
                    "employee":frm.doc.employee,
	...
	calculate_total_days: function(frm) {
		if(frm.doc.from_date && frm.doc.to_date && frm.doc.employee && frm.doc.leave_type) {
			...
			return frappe.call({
				method: 'ecs_vim.ecs_vim.doctype.leave_request.leave_request.get_number_of_leave_days',
				args: {
					"employee": frm.doc.employee,
					"leave_type": frm.doc.leave_type,
					"from_date": frm.doc.from_date,
	...
	set_leave_approver: function(frm) {
		if(frm.doc.employee) {
			// server call is done to include holidays in leave days calculations
			return frappe.call({
				method: 'ecs_vim.ecs_vim.doctype.leave_request.leave_request.get_leave_approver',
				args: {
					"employee": frm.doc.employee,
				},
				callback: function(r) {
