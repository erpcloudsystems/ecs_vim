...
class EarlyGoingRequest(Document):
    def onload(self):
    ...
    def on_submit(self):
        ...
        if self.get("status") == "Open":
            frappe.throw(
                _(
                    "Only Early Going Request with status 'Approved' and 'Rejected' can be submitted"
                )
            )
    ...
    def validate_duplicate(self):
        ...
        if duplicate:
            frappe.throw(
                _(
                    "Employee has Already Applied for Early Going Request for this date {}".format(
    ...
    def update_attendance(self, short_time):
        ...
        att_doc.early_going_minutes = short_time
        att_doc.db_update()
    ...
    def att_application(self, validate):
        ...
        if not attendance.get("attendance_out_punch"):
            if validate:
                frappe.throw(_("Employee Punch Not Found"))
            else:
                return

        self.actual_number_of_minutes_late = attendance.get("early_going_minutes")

    def employee_checkin(self):
        from ecs_vim.custom_script.employee_checkin.employee_checkin import (
            get_employee_shift,
        )
        from ecs_vim.custom_script.attendance.attendance import early_going
        ...
        first_punch = logs[-1] if logs else {}
        first_in_punch = first_punch.get("time")
        shift = frappe._dict(
            {
                "enable_exit_grace_period": current_shift.shift_type.enable_exit_grace_period,
                "early_exit_grace_period": current_shift.shift_type.early_exit_grace_period,
                "end_datetime": current_shift.end_datetime,
            }
        )

        self.actual_number_of_minutes_late = early_going(shift, first_in_punch)
