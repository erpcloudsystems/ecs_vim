...
# from ecs_vim.custom_script.attendance.attendance import get_ot
from ecs_vim.ecs_vim.doctype.ot_planner.ot_planner import applied_ot_rule
from ecs_vim.custom_methods import get_comp_name
from ecs_vim import *
...
class OTRequest(Document):
    ...
    def on_submit(self):
        if self.get("status") == "Open":
            frappe.throw(
                _(
                    "Only OT Request with status 'Approved' and 'Rejected' can be submitted"
                )
            )
    ...
    def attendance_processed(self):
        attendance = """SELECT *
            ...
            self.get("applicant"), self.get("ot_request_date")
        )
    ...
    def ot_planner(self):
        duplicate = """SELECT *
			...
			WHERE docstatus < 2
			AND employee = '{}'
			AND '{}' BETWEEN from_date AND to_date""".format(
            self.get("applicant"), self.get("ot_request_date")
        )
        ...
        if duplicate:
            ...
            frappe.throw(
                _(
                    "Employee has OT Planned between {} and {}".format(
                        formatdate(duplicate.get("from_date")),
                        formatdate(duplicate.get("to_date")),
                    )
                )
            )

    def update_attendance(self, ot_min=0):
        from ecs_vim.custom_script.attendance.attendance import process_attendance
        ...
        process_attendance(
            doc=att_doc,
            ot_request=ot_min,
            overtime_without_punch=self.get(
    ...
    def ot_worked(self):
        ot_rule = applied_ot_rule(
            employee=self.get("applicant"), to_date=self.get("ot_request_date")
        )
        ...
        start_date_time = "{} {}".format(
            self.get("ot_request_date"), self.get("from_time")
        )
        ...
        ot_working_sec = (
            get_datetime(end_date_time) - get_datetime(start_date_time)
        ).total_seconds()
        ...
        ot_working_min = abs(ot_working_min / (ot_rule_doc.get("ot_slab") or 1)) * (
    ...
    def get_ot_amt(self, ot_rule):
        from ecs_vim.custom_script.attendance.attendance import weekoff_holiday

        start_date = get_first_day(self.get("ot_request_date"))
        end_date = get_last_day(start_date)
        company = frappe.db.get_value(
            "Employee", {"name": self.get("applicant")}, "company"
        )
        ...
        weak_off, _shift = weekoff_holiday(
            self.get("applicant"), getdate(self.get("ot_request_date"))
        )
        ...
        if weak_off:
            ot_rate = (
                ot_rule.get(
                    "non_working_day_ot_rate"
                    if weak_off[0]["is_weekoff"]
                    else "holiday_ot_rate"
                )
                or 0
            )
        ...
        basic_amt = sum(
            [
                (
                    earning_deduction.get("earnings", {})
                    .get(get_comp_name(abbr, 0), {})
                    .get("default_amount")
                    or 0
                )
        ...
        from ecs_vim.custom_script.employee_checkin.employee_checkin import (
            get_employee_shift,
        )

        current_shift = get_employee_shift(
            employee=self.get("applicant"),
            for_date=getdate(self.get("ot_request_date")),
            consider_default_shift=True,
            next_shift_direction="forward",
        )
        daily_working_hrs_minutes = (
            current_shift.end_datetime.replace(second=0)
            - current_shift.start_datetime.replace(second=0)
        ).total_seconds() / 60
        ...
        frappe.errprint(
            [basic_amt, daily_working_hrs_minutes, ot_rate, (self.get("ot_hours") or 0)]
        )
        self.ot_value = (
            (basic_amt / daily_working_hrs_minutes)
            * ot_rate
            * (self.get("ot_hours") or 0)
        )
    ...
    def validate_sp(self):
        overtime_without_punch = (
            self.get("apply_overtime_without_any_attendance_punches") or 0
        )

        if self.is_single_punch() < 2 and overtime_without_punch == 0:
            frappe.throw(
                _(
                    "OT Request for this date can be created only with 'Apply overtime without any attendance punches' as the system should have at least 2 punches"
                )
            )

    def get_attedance(self):
        from ecs_vim.custom_script.attendance.attendance import ot_worked

        attendance_list = frappe.db.sql(
            """select * from `tabAttendance` where docstatus=1 and employee="{0}" and attendance_date ='{1}'
			 and ot_hours>0""".format(
                self.get("applicant"), self.get("ot_request_date")
            ),
            as_dict=True,
        )
        if attendance_list:
            attendance_details = ot_worked(
                employee=self.get("applicant"),
                attendance_date=getdate(self.get("ot_request_date")),
                error=not bool(validate or 0),
            )
            ...
            self.to_time = get_time(
                ot_start_time + timedelta(seconds=ot_hours * 3600)
            ).strftime("%H:%M:%S")
    ...
    def attendance_ot(self, validate=0):
        ...
        if attendance_list:
            ...
            if int(validate or 0) == 0:
                ...
                if (
                    attendance_details.get("out_time")
                    and self.to_time
                    and get_time(self.to_time)
                    > get_time(attendance_details.get("out_time"))
                ):
                    self.to_time = get_time(
                        attendance_details.get("out_time")
                    ).strftime("%H:%M:%S")
                ...
                if not attendance_details.get(
                    ...
                    if get_datetime(start_date_time) < ot_start_time:
                        frappe.throw(
                            _("OT Request from Time Should be greater then {}").format(
                                ot_start_time
                            )
                        )
                overtime_without_punch = self.get(
                ...
                if overtime_without_punch or 0:
                    return
                ...
                if not otr_start_time == shift_end_time:
                    frappe.throw(
                        _("OT Request Start Time Should be {}").format(shift_end_time)
                    )
                ...
                if otr_end_time > last_punch:
                    frappe.throw(
                        _("OT Request To Time Should be less then {}").format(
                            last_punch
                        )
                    )
            import math
