...
class EmployeePortal(Document):
    ...
    def employee_detail_json(self):
        ...
        leave_data = (
            get_leave_details(self.get("employee"), nowdate())
        ...
        emp_detail["Leave History"] = {
            "method": "leave_history_tab",
            "data": self.leave_history(),
        }
        emp_detail["Salary Details"] = {
            "method": "cardview_tab",
            "data": self.salary_detail(),
        }

        return json.dumps(
            {"employee_detail": emp_detail, "profile_detail": profile_data}
        )

    def employee_profile(self):
        employee = """select * from `tabEmployee` where name = '{0}'""".format(
            self.get("employee")
        )
        ...
        bi_data["Date of Joining"] = str(
            formatdate(employee.get("date_of_joining")) or ""
        )
        ...
        bi_data["Driving License Issue Date"] = (
            employee.get("driving_license_issue_date") or ""
        )
        bi_data["Driving License Expiry Date"] = (
            employee.get("driving_license_expiry_date") or ""
        )
        ...
        return {
            "profile_img": employee.get("image"),
    ...
    def get_payroll_period(self, payroll_period):
        if payroll_period:
            payroll = """SELECT * FROM `tabPayroll Period` WHERE name = '{}'""".format(
                payroll_period
            )
        else:
            payroll = """SELECT `PP`.*
				...
				WHERE `PP`.`start_date` = '{}'
				AND `PP`.`end_date` = '{}'""".format(
                self.get("employee"), get_first_day(getdate()), get_last_day(getdate())
            )
        ...
        return (
            payroll[0]
            if payroll
            else frappe.throw("Payroll Period is not configured for Current Month")
        )
    ...
    def attendance_detail(self, payroll_dict):
        attendance = """select COUNT(IF(status='On Leave' and leave_type='{3}', status, NULL)) AS vacation,
			...
			where employee = '{0}'
			and docstatus < 2
			and attendance_date between '{1}' and '{2}'""".format(
            self.get("employee"),
            payroll_dict.get("actual_start_date"),
            payroll_dict.get("actual_end_date"),
            get_leve_name("V"),
            get_leve_name("WO"),
            get_leve_name("H"),
        )
        ...
        attendance_data["Total Late coming hrs"] = convert(
            attendance.get("late_coming_minutes") or 0
        )
        attendance_data["Total Early going hrs"] = convert(
            attendance.get("early_going_minutes") or 0
        )

        return {
            "data": attendance_data,
            "period": "{} - {}".format(
                formatdate(payroll_dict.get("actual_start_date")),
                formatdate(payroll_dict.get("actual_end_date")),
            ),
            "link": {
                ...
                "option": {
                    ...
                    "start_date": payroll_dict.get("actual_start_date").strftime(
                        "%m-%d-%Y"
                    ),
    ...
    def salary_slip(self, payroll_dict):
		...
		and end_date = '{2}'""".format(
            self.get("employee"),
            payroll_dict.get("start_date"),
            payroll_dict.get("end_date"),
        )
        ...
        sal_data["Basic Salary"] = (
            self.salary_structure(
                datetime.strftime(payroll_dict.get("end_date"), "%m"),
                datetime.strftime(payroll_dict.get("end_date"), "%Y"),
            ).get("base")
            or 0
        )
        ...
        salary_detail = """SELECT salary_component, IF(parentfield='earnings', amount, 0) AS 'earning', IF(parentfield='deductions', amount, 0) AS 'deduction'
			...
			WHERE parent = '{}'
			AND amount > 0
			ORDER BY parentfield DESC, idx""".format(
            sal_slip.get("name")
        )
        ...
        return {
            ...
            "period": "{} - {}".format(
                formatdate(payroll_dict.get("start_date")),
                formatdate(payroll_dict.get("end_date")),
            ),
            "link": {
                "label": "Print Salary Slip",
                "action": "custom_print_format",
                "option": {"doctype": "Salary Slip", "name": sal_slip.get("name")},
            },
        }
