...
from ecs_vim.custom_script.leave_application.leave_application import (
...
class VacationLeaveApplication(Document):
    def validate(self):
        if self.validate_ss_processed(
            self.employee_id, self.from_date
        ) or self.validate_ss_processed(self.employee_id, self.to_date):
            frappe.throw(
                "Can not create or edit Vacation leave application, Salary is already processed for this month"
            )
        ...
        if self.total_leave_days > self.eligible_days:
            if self.eligible_days > 0:
                special_leav = flt(
                    self.total_leave_days - self.eligible_days,
                    self.precision("eligible_days"),
                )
            else:
                special_leav = self.total_leave_days

        if special_leav > 0:
            msg = "Employee {} has applied for Special leave for {} days".format(
                self.get("employee_id"), special_leav
            )
            if self.warning_msg:
    ...
    def before_save(self):
        ...
        if int(workflow_del) == 1:
            ...
            if emp_doc.user_id:
                ...
                if (
                    len(workflow) > 0
                    and self.doctype == "Vacation Leave Application"
    ...
    def before_cancel(self):
        if self.validate_ss_processed(
            ...
            frappe.throw(
                "Can not cancel Vacation leave application, Salary is already processed for this month"
            )
    ...
    def cancel_leave_application(self, status):
        sql = "select name, docstatus from `tabLeave Application` where vacation_leave_application = '{0}'".format(
            self.name
        )
    ...
    def new_get_accrual_till(self):
        if self.employee_id and self.from_date:
            ...
            result_dict = calculate_vacation(
                employee=self.employee_id, from_date=self.from_date, doctype=self.name
            )
...
def check_vacation_rule(employee_dict):
    ...
    if emp_date:
        ...
        if emp_date["vacation_rule"]:
            ...
            frappe.throw(
                "Vacation Leave Rule is not assigned to Employee '{0}'".format(
...
def get_vacation_rule(employee_dict):
    ...
    if get_rule:
        ...
        if (
            employee_dict["monthly_credit"]
            and employee_dict["iterate_loop"] == 1
            and employee_dict["max_range"] < to_date
            and employee_dict["eligible_days"] == 0
        ):
            employee_dict["warning_msg"] = _(
                "As Per Vacation Rule, Employee {} is eligible from {}".format(
                    employee_dict["employee"], formatdate(to_date)
                )
            )
            frappe.msgprint(employee_dict["warning_msg"])

        if get_rule["carry_forward_allowed"]:
            if not employee_dict.__contains__("carryforward") or (
                employee_dict["carryforward"]
                and add_days(employee_dict["carryforward"]["to_date"], 1)
                == employee_dict["start_date"]
            ):
                employee_dict["carryforward"] = {
                    "from_date": employee_dict["start_date"],
                    "to_date": add_days(to_date, -1),
                    "carryforward_day": get_rule["carryforward_day"],
                    "carryforward_expiry": add_months(
                        to_date, get_rule["carryforward_expiry"]
                    )
                    if get_rule["carryforward_expiry"] > 0
                    else None,
                }
        else:
            employee_dict["carryforward"] = {}
        ...
        # redirect condition
        if (
            get_rule["redirect_after_year"]
            and float(get_rule["redirect_after_year"]) > 0
        ):
            emp_new_effective_dt = add_years(
                employee_dict["applied_date"], get_rule["redirect_after_year"]
            )
            employee_dict["rule"]["redirect_date"] = emp_new_effective_dt
            new_count_diff = date_diff(
                employee_dict["start_date"], str(emp_new_effective_dt)
            )
        ...
        if not employee_dict["monthly_credit"]:
            ...
            between_date = add_days(
                employee_dict["max_range"], -1 * employee_dict["day_adjust"]
            )
            date_between_range(employee_dict, between_date)
...
def date_between_range(employee_dict, between_date, encash_abj=0):
    if employee_dict["start_date"] <= between_date <= employee_dict["to_date"]:
        ...
        if encash_abj:
            employee_dict["encashed_max_range"] = add_days(
                employee_dict["encashed_max_range"], -1
            )
...
def rule_change_date(employee_dict):
    rule_change = rule_change_select().format(
        employee_dict["employee"],
        add_days(employee_dict["start_date"], 1),
        employee_dict["to_date"],
    )
    ...
    if rule_change:
        ...
        if rule_change["docstatus"] == 0 and employee_dict["ignore_msg"] == 0:
            frappe.throw(
                "Vacation Rule Modification Is Not Submitted For Application {0}".format(
                    rule_change["name"]
                )
            )
...
def get_upl_leave(employee_dict, start_date, end_date):
    get_upl_sql = """SELECT `l`.`from_date`, `l`.`to_date`
		...
		AND `l`.`employee` = '{0}'
		AND `l`.`from_date` BETWEEN '{1}' AND '{2}'""".format(
        employee_dict["employee"], start_date, end_date
    )
...
def calc_accural_rcd(employee_dict, ignore_rejoining):
    actual_to_date = add_months(
        employee_dict["start_date"], employee_dict["rule"]["range_date"]
    )
    actual_day_diff = days360(
        getdate(employee_dict["start_date"]), getdate(actual_to_date)
    )
    ...
    if (
        employee_dict["monthly_credit"]
        and employee_dict["encashed_max_range"] < employee_dict["to_date"]
    ):
        return

    day_diff = days360(
        getdate(employee_dict["start_date"]),
        getdate(add_days(employee_dict["to_date"], 1)),
    )
...
def carryforward_adj(employee_dict):
    if not employee_dict["carryforward"]:
        return
    ...
    if (
        employee_dict.get("carryforward_day_expire")
        and deducat_expired_day > 0
        and employee_dict["start_date"]
        <= employee_dict.get("carryforward_day_expire")
        <= employee_dict["to_date"]
    ):
        ...
        employee_dict["expired_carryforward_on"] = employee_dict[
            "carryforward_day_expire"
        ]
    ...
    if employee_dict["to_date"] == carryforward_rule["to_date"]:
        ...
        if (
            carryforward_rule["carryforward_expiry"]
            and carryforward_rule["carryforward_expiry"]
            > employee_dict["encashed_max_range"]
        ):
            return
        ...
        if (
            carryforward_rule["carryforward_day"] > 0
            and employee_dict["eligible_days"] > carryforward_rule["carryforward_day"]
        ):
            employee_dict["carryforward_flag"] = 1
            adjust_day = (
                employee_dict["eligible_days"] - carryforward_rule["carryforward_day"]
            )
        ...
        if (
            employee_dict["eligible_days"] > 0
            and carryforward_rule["carryforward_expiry"]
        ):
            employee_dict["carryforward_day_expire"] = carryforward_rule[
                "carryforward_expiry"
            ]
...
def vacation_select(ignore_rejoining):
    return """
		...
		OR (`V`.`docstatus` = 0
			AND `V`.`status` = 'Rejected'))""" % (
        "IFNULL(`VR`.`vacation_rejoining_date`, DATE_ADD(`V`.`to_date`, INTERVAL 1 DAY))"
        if ignore_rejoining
        else "`VR`.`vacation_rejoining_date`",
        "IF(`VR`.`vacation_rejoining_date`, `VR`.`docstatus`, 1)"
        if ignore_rejoining
        else "`VR`.`docstatus`",
    )
...
def prev_vacation(employee_dict, ignore_rejoining):
    ...
    for v_row in vacation:
        ...
        if employee_dict["ignore_msg"] == 0:
            validate_rejoing_appl(v_row)

        if employee_dict.get("carryforward_day_expire") and v_row.get(
            "from_date"
        ) < employee_dict.get("carryforward_day_expire"):
...
def validate_rejoing_appl(data):
    if data["v_docstatus"] == 0:
        frappe.throw(
            "Vacation Leave Application Is Not Approved For Application {0}".format(
                data["name"]
            )
        )
    ...
    if not data["vacation_rejoining_date"] and not data["r_docstatus"]:
        frappe.throw(
            "Please Apply Vacation Rejoining Application For Application {0}".format(
                data["name"]
            )
        )
    ...
    if data["vacation_rejoining_date"] and data["r_docstatus"] == 0:
        frappe.throw(
            "Vacation Rejoining Application Is Not Approved For Application {0}".format(
                data["name"]
            )
        )
...
def validate_vacation_date(employee_dict, ignore_rejoining):
    ...
    vacation = "{0} \
		...
		LIMIT 1".format(
        first_select, employee_dict["employee"], employee_dict["docname"]
    )
    vacation = frappe.db.sql(vacation, as_dict=True)
    ...
    if vacation:
        vacation = vacation[0]
        if employee_dict["ignore_msg"] == 0:
            validate_rejoing_appl(vacation)
        get_upl_leave(
            employee_dict,
            vacation["vacation_rejoining_date"],
            employee_dict["max_range"],
        )
        employee_dict["total_working"] = days360(
            getdate(vacation["vacation_rejoining_date"]), employee_dict["max_range"]
        ) - get_exclude_count(employee_dict)
        employee_dict["last_vacation"] = (
            str(vacation["from_date"]) + " to " + str(vacation["to_date"])
        )
        ...
        if ignore_rejoining:
            if (
                ...
                <= employee_dict["encashed_max_range"]
                <= vacation["vacation_rejoining_date"]
                and employee_dict["ignore_msg"] == 0
            ):
                frappe.throw(
                    "Leave Encashment Range Cannot fall between Vacation Leave Application"
                )

            if vacation["vacation_rejoining_date"] >= employee_dict["max_range"]:
                employee_dict["max_range"] = add_days(
                    vacation["vacation_rejoining_date"], 1
                )
        ...
        if (
            ...
            frappe.throw(
                "Leave cannot be applied for date on or before {0}".format(
                    formatdate(vacation["vacation_rejoining_date"])
                )
            )
        ...
        get_upl_leave(
            employee_dict, employee_dict["start_date"], employee_dict["max_range"]
        )
        employee_dict["total_working"] = days360(
            getdate(employee_dict["start_date"]), employee_dict["max_range"]
        ) - get_exclude_count(employee_dict)
...
def leave_encashed(employee_dict, order=None):
    encashed_leav = vacation_encash_select(order).format(
        employee_dict["employee"],
        employee_dict["start_date"],
        employee_dict["to_date"] if employee_dict.__contains__("to_date") else "",
        employee_dict["docname"],
    )
    ...
    for e_row in encashed_leav:
        if e_row["docstatus"] == 0 and employee_dict["ignore_msg"] == 0:
            frappe.throw(
                "Vacation Leave Encashment Is Not Approved For Application {0}".format(
                    e_row["name"]
                )
            )

        if (
            getdate(e_row["to_date"]) >= employee_dict["max_range"]
            and employee_dict["ignore_msg"] == 0
        ):
            frappe.throw(
                "Leave cannot be applied for date on or before {0}".format(
                    formatdate(e_row["to_date"])
                )
            )
        ...
        if not order and e_row["docstatus"] == 1 and float(e_row["pay_days"]) > 0:
            ...
            if employee_dict.get("carryforward_day_expire") and e_row.get(
                "to_date"
            ) < employee_dict.get("carryforward_day_expire"):
...
def calculate_vacation(
    employee, from_date, doctype=None, ignore_rejoining=0, ignore_msg=0
):
    employee_dict = dict()
    ...
    if employee_dict.__contains__("applied_rule") and employee_dict.__contains__(
        "start_date"
    ):
        while True:
            employee_dict["iterate_loop"] += 1
            if employee_dict["iterate_loop"] == 100:
                break
            ...
            get_upl_leave(
                employee_dict, employee_dict["start_date"], employee_dict["to_date"]
            )
            ...
            # dynamic credit
            if employee_dict["start_date"] >= add_days(
                employee_dict["max_range"], -1 * employee_dict["day_adjust"]
            ):
                break
    ...
    return {
        "eligible_days": employee_dict["eligible_days"],
        "last_vacation": employee_dict["last_vacation"]
        if employee_dict.__contains__("last_vacation")
        else None,
        "applied_rule": employee_dict["applied_rule"],
        "working_days": employee_dict["total_working"],
        "warning_msg": employee_dict["warning_msg"]
        if employee_dict.__contains__("warning_msg")
        else None,
...
def filter_employee(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql(employee_list, {"txt": "%%%s%%" % txt})
