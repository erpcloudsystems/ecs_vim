...
from ecs_vim.custom_methods import get_comp_name, get_leve_name
from ecs_vim.custom_script.salary_slip.salary_slip import (
    get_actual_structure,
    get_payroll_period,
)
from ecs_vim.ecs_vim.doctype.vacation_leave_application.vacation_leave_application import (
    calculate_vacation,
)
from ecs_vim.ecs_vim.doctype.vacation_leave_encashment.vacation_leave_encashment import (
...
class ServiceClosing(Document):
    ...
    def on_cancel(self):
        if self.salary_slip:
            sql = (
                "select * from `tabSalary Slip` where name = '" + self.salary_slip + "'"
            )
        ...
        if self.journal_entry:
            sql = (
                "select * from `tabJournal Entry` where name = '"
                + self.journal_entry
                + "'"
            )
    ...
    def update_employee(self, status):
        ...
        employee.relieving_date = (
            self.get("termination_date") if status and status == "Left" else None
        )
        employee.save()
    ...
    def mandatory_fild(self):
        if not self.payment_account or not self.expense_account:
            frappe.throw(
                "Please fill the mandatory field ['Expense Account', 'Payment Account']"
            )
    ...
    def salary_structure_detail(self):
        ...
        if not service_closing:
            ...
            if not struct_assig:
                frappe.throw(
                    _("There is no Salary Structure for the selected Employee")
                )
            ...
            self.basic_salary = (
                frappe.db.get_value(
                    "Salary Structure Assignment", {"name": struct_assig}, "base"
                )
                or 0
            )
            self.gross_pay_amount = sum(
                [row.get("default_amount") for row in earnings.values()]
            )
        else:
    ...
    def cal_date(self):
        ...
        return relativedelta(
            getdate(self.get("termination_date")), getdate(self.get("joining_date"))
        )
    ...
    def component_amount(self, name):
        ...
        if frappe.db.get_value(
            "Benefit Type for EOS", name, "is_not_applicable_all_components"
        ):
    ...
    def calculate_service_awd(self):
        ...
        award_list = """SELECT `S`.*
			...
			WHERE `S`.`row_idx` = 1""".format(
            self.get("company"),
            self.get("termination_date"),
            self.get("termination_type"),
            day_working,
        )
        ...
        for _slab in slab:
            ...
            eval_dict = {"EDAYS": day_working, "ECOMP": sarvice_awd_amt}

            service_amt = eval(str(_slab.get("formula")), eval_dict)
            list_return.append(
                {
                    "service_award_rule": _slab.get("parent"),
    ...
    def validate_adjustment(self, ignore):
        ...
        for row in self.get("service_closing_adjustment"):
            ...
            if additional_amount < 0:
                frappe.throw(
                    _(
                        "Addition Amount Cannot be Less then Zero at Row {}".format(
                            row.get("idx")
                        )
                    )
                )
            ...
            if deduction_amount < 0:
                frappe.throw(
                    _(
                        "Deduction Amount Cannot be Less then Zero at Row {}".format(
                            row.get("idx")
                        )
                    )
                )
            ...
            if additional_amount > 0 and deduction_amount > 0:
                frappe.throw(
                    _(
                        "Both The Amount Cannot Be Assigned at Row {}".format(
                            row.get("idx")
                        )
                    )
                )
    ...
    def calc_total_amount(self):
        total_amount = (
            (self.get("total_leave_encashment_amount") or 0)
            + (self.get("sa_total_amount") or 0)
            + (self.get("total_salary_amount") or 0)
            + (self.get("total_additional_amount") or 0)
            - (self.get("total_deduction_amount") or 0)
        )
    ...
    def working_days(self):
        days_working = (
            date_diff(self.get("termination_date"), self.get("joining_date")) + 1
        )
        ...
        absent = """SELECT COUNT(*) days
			...
			AND attendance_date BETWEEN '{}' AND '{}'""".format(
            self.get("employee"), self.get("joining_date"), self.get("termination_date")
        )
    ...
    def leave_balance(self):
        ...
        vacation_rule = frappe.db.get_value(
            "Employee", self.get("employee"), "vacation_rule"
        )
        if vacation_rule:
            leave_ledg = calculate_vacation(
                employee=self.get("employee"), from_date=self.get("termination_date")
            )
            ...
            leave_list.append(
                {
                    "leave_type": get_leve_name("V"),
                    "leave_balance": leave_days,
                    "encashment_amount": vacation_encash_amount(earnings, leave_days)
                    - vacation_encash_amount(deductions, leave_days),
                }
            )
        ...
        self.total_leave_encashment_amount = sum(
            [row.get("encashment_amount") for row in leave_list]
        )
    ...
    def calculate_sal(self):
        ...
        sql = (
            "select sum(total_payment) as total_pay, (total_interest_payable) as total_interest , sum(total_amount_paid) as total_paid from `tabLoan` where applicant = '"
            + self.employee
            + "' and docstatus = 1 and repay_from_salary = 1"
        )
        ...
        self.total_salary_amt = (
            doc.get("gross_pay")
            - doc.get("total_deduction")
            - doc.get("total_loan_repayment")
        )
        self.loan_amount = loan_amount
        self.loan_advance = loan_advance
        self.ot_amount = round(
            float(add_component_val(doc.earnings, [get_comp_name("OT")])), 2
        )
        ...
        deleted_document = frappe.db.sql(
            "select name from `tabDeleted Document` where deleted_doctype = 'Salary Slip' and deleted_name = '"
            + doc.name
    ...
    def attendance_count(self):
        ...
        attendance = """select *
            ...
            self.employee, from_date, to_date
        )
        attendance = frappe.db.sql(attendance, as_dict=1)

        self.present_days = sum(
            (1 if (row.status == "Present" or row.status == "Absent") else 0)
    ...
    def make_jv_entry(self):
        ...
        journal_entry.user_remark = _(
            "Accrual Journal Entry for Settlement for Employee " + self.employee + ""
        )
