...
frappe.ui.form.on("Vacation Rejoining", {
	setup: (frm) => {
		...
		frm.set_query('vacation_leave_application', () => {
			return {
				query: "ecs_vim.ecs_vim.doctype.vacation_rejoining.vacation_rejoining.get_leave",
				filters: {
		...
		frm.set_query('employee_id', () => {
			return {
				query: "ecs_vim.ecs_vim.doctype.vacation_leave_application.vacation_leave_application.filter_employee"
			}
		});
	},
