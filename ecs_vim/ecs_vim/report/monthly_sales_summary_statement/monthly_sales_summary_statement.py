...
def get_conditions(filters):
	conditions = ""
	conditions += " and parent.posting_date BETWEEN DATE_FORMAT(CURDATE(), '%Y-%m-01') AND CURDATE()"
	return conditions
...
def get_pos_invoice_data(conditions,filters):
	str=frappe.db.sql(f"""select distinct pos.warehouse,sum(parent.net_total) as net_total,ifnull(sum(trip.trip_count),0) as trip_count,ifnull(sum(i.non_sharable_slot),0) non_sharable_slot,
    ...
    where parent.docstatus = 1 {conditions} group by  pos.warehouse""" , as_list=1)
