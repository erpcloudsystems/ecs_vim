...
import json
from ecs_vim.ecs_vim.report.utils import get_columns, get_values_list
...
def execute(filters=None):
    ...
    sql = "call VAT_Report({0}, {1}, {2}, {3})".format(
        json.dumps(filters.get("company")),
        json.dumps(filters.get("from_date")),
        json.dumps(filters.get("to_date")),
        json.dumps(filters.get("grouping_for_vat_report")),
    )
    frappe.errprint(str(sql))
    dict = frappe.db.sql(
        "call VAT_Report({0}, {1}, {2}, {3})".format(
            json.dumps(filters.get("company")),
            json.dumps(filters.get("from_date")),
            json.dumps(filters.get("to_date")),
            json.dumps(filters.get("grouping_for_vat_report")),
