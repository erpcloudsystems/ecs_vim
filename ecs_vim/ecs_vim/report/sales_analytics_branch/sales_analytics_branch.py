...
class Analytics(object):
		...
		self.columns.append(
			{
				...
				"fieldname": "branch",
				"fieldtype": "Link",
				"options": "Dimension Branch",  # Adjust the options based on your actual doctype for Branch
				"width": 120,
			}
		)	
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		if self.filters.doc_type == "Sales Invoice":
			branch = "dimension_branch"
		else:
			branch = "branch"
		self.entries = frappe.get_all(
			self.filters.doc_type,
            fields=[entity, entity_name, branch, value_field, self.date_field],
			filters={
				"docstatus": 1,
		...
		frappe.msgprint(str(self.entries))
		self.entity_names = {}
		for d in self.entries:
			...
			'branch': d.branch  # Add branch to entity_names
		})
	...
	def get_rows(self):
		...
		for entity, period_data in self.entity_periodic_data.items():
			#Adding branch in row
			row = {
				"entity": entity,
				"entity_name": self.entity_names.get(entity, {}).get('entity_name') if hasattr(self, "entity_names") else None,
                "branch": self.entity_names.get(entity, {}).get('branch'),  # Add this line for the "Branch" field
			}
			total = 0
			for end_date in self.periodic_daterange:
				period = self.get_period(end_date)
			...
			#row["branch"] = period_data.get("branch")
			row["total"] = total
