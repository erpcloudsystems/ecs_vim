...
erpnext.SerialNoBatchSelector = Class.extend({
                            ...
                            callback: function (r) {
                                ...
                                if(data_list){
                                    ...
                                    for(var i=0;i<data_list.length;i++){
                                        batches.grid.df.data.push({
                                            'batch_no': data_list[i].batch_no,
                                            'expiry_date': data_list[i].expiry_date,
                                            'selected_qty': data_list[i].qty,
                                            'available_qty': data_list[i].available_qty
                                        });
        ...
        frappe.call({
            ...
            callback: function (r) {
                ...
                if(data_list){
                    ...
                    for(var i=0;i<data_list.length;i++){
                        dialog.fields_dict.batches.df.data.push({
                            'batch_no': data_list[i].batch_no,
                            'expiry_date': data_list[i].expiry_date,
                            'selected_qty':0,
                            'available_qty': data_list[i].available_qty
                        ...
                        if (me.frm.doc.items){
                            me.frm.doc.items.forEach(data => {

                                    var d=dialog.fields_dict.batches.df.data;
                                    for(var j=0;j<d.length;j++){
                                        if(d[j]["batch_no"]==data.batch_no)
                                        {
                                            d[j]["selected_qty"]=data.qty
                                        }
                                    }
			...
			if (this.has_batch && !this.has_serial_no && d.batch_no) {
				this.frm.doc.items.forEach(data => {
					if(data.item_code == d.item_code) {
                        var dlist=this.dialog.fields_dict.batches.df.data;
	...
	validate: function() {
		...
		if(this.has_batch && !this.has_serial_no) {
            ...
            //         qtycal=qtycal+(batch.selected_qty==''?0 :batch.selected_qty)
            //         if(batch.selected_qty !=batch.available_qty){
                    
            //             totalqty=totalqty+(batch.selected_qty==''?0 :batch.selected_qty)
            //             itemcount++;
            //            console.log(itemcount,totalqty,'count')
            //         }
                    
            //     }
                
			// });
            // console.log(values.batches.map,'map')
            // var data=values.batches;
            
			values.batches.map((batch, i) => {
                
				if (batch.selected_qty=='')
				{
					batch.selected_qty =0
				}
              
                if(batch.selected_qty>=0){
					qtycal=qtycal+(batch.selected_qty)
                    
					if (batch.selected_qty < batch.available_qty)
					{
						if (batch.idx != values.batches.length){
							 
							if ((values.batches[i+1].selected_qty == '' ? 0 : values.batches[i+1].selected_qty) != 0)
							{
                                var qty=qtycal+values.batches[i+1].selected_qty
                                if (batch.available_qty < qty){
                                    var extraqty=qty-batch.available_qty
                                    frappe.throw(__("Allocate "+batch.available_qty+" Quantity to "+batch.batch_no+" and Remaining "+extraqty+" To Other Batch"))
                                }else{
                                    frappe.throw(__("Allocate all "+qty+" Quantity to "+batch.batch_no+""))
                                }
					...
					// if(batch.selected_qty !=batch.available_qty){
					// 	// console.log(values.batches[i+1].selected_qty)
                ...
                if(qtycal==0){
                    frappe.throw(__("Enter quantity for atleast one batch"))
                }
                // if(totalqty<batch.available_qty){
                    
                //     itemcount=itemcount-1;
                //     console.log(itemcount,'sdfd')
                // }
                // if (batch.selected_qty<=batch.available_qty){
                //     totalqty=totalqty-batch.selected_qty;
                // }
                // if(qtycal>0)
                // {

                // }
                // if (batch.selected_qty==0 && batch.available_qty>=totalqty && totalqty>0 ){
                //     frappe.throw(__("Allocate all "+totalqty+" Quantity to "+batch.batch_no+""))
                // }
                // if(batch.selected_qty >0 && itemcount>1){
                //     if(batch.available_qty>totalqty){
                //         console.log('hiii')
                //         frappe.throw(__("Allocate all "+totalqty+" Quantity to "+batch.batch_no+""))
                //     }
                // }
                // if(batch.available_qty>=totalqty && itemcount >1){
                //     console.log('hsiii')
                //     frappe.throw(__("Allocate all "+totalqty+" Quantity to "+batch.batch_no+""))
                // }
				...
				fields: [
					...
					},
					{
						...
						change: function () {
                            ...
                            // this.grid.wrapper.find('.grid-add-row').hide()
