...
frappe.search.utils = {
	...
	get_recent_pages: function(keywords) {
		...
		frappe.route_history.forEach(function(route, i) {
			...
			} else if(['List', 'Tree','Calendar', 'Workspaces', 'query-report'].includes(route[0]) || route[2]==='Report') {
				if(route[1]) {
		...
		find(values, keywords, function(match) {
			...
			} else if (['List', 'Tree','Calendar', 'Workspaces', 'query-report'].includes(match[1][0]) && (match[1].length > 1)) {
				...
				if (in_list(frappe.boot.single_types, item)) {
					...
					if (in_list(frappe.boot.user.can_create, item)) {
					 ...
					 if (in_list(frappe.boot.treeviews, item)) {
						...
						if (in_list(frappe.boot.calendars, item)) {
							out.push(option("Calendar", [ item.toLowerCase().split(' ').join('-'),"view","calendar","default"], 0.05));
