...
from ecs_vim.api import get_product_info_for_website as get_product_info
from erpnext.e_commerce.shopping_cart.product_info import get_product_info_for_website
from frappe.utils import cstr
from ecs_vim.api import sign_up
    ...
    for item in result:
        ...
        if product_info:
            item.formatted_price = (
                product_info["price"].get("formatted_price")
...
def get_branch_list(brand):
    branch_list = frappe.db.get_list(
        "Dimension Branch", filters={"brand": brand}, pluck="name"
    )
    return branch_list
...
def sign_up(email, full_name, mobile, city, national_id=None):
    ...
    user = frappe.db.get("User", {"email": email}) or frappe.db.get(
...
def is_available(item_code=None, visit_date=None, branch=None):
    if not item_code:
        return {"message": "Please provide item code"}
    if not visit_date:
        return {"message": "Please provide visit date"}
    ...
    if branch:
        ...
        if block_list and len(block_list) > 0:
            if block_list[0]["applicable_for"] == 1:
                return {
                    "available": False,
                    "message": "This event is blocked for the selected event from {} to {} due to {}".format(
                ...
                return {
                    "available": False,
                    "message": "No events are performed from {} to {} due to {}".format(
                        block_list[0]["block_from"],
                        block_list[0]["block_to"],
                        block_list[0]["reason"],
                    ),
            ...
            if ev_date.date() == dates.date:
                if dates.rate_applicable != applicable_for:
                    if applicable_for == "Weekday":
                        return {
                            "available": False,
                            "message": "Weekday event only, please select Weekend event or select Weekday date",
                        }
                    else:
                        return {
                            "available": False,
                            "message": "Weekend event only, please select Weekday event or select Weekend date",
                        }
                else:
                    special_day = True
    ...
    if frappe.session.user != "Guest":
        ...
        if user:
            ...
            mobile_no = frappe.db.get_value(
                "User", {"email_id": frappe.session.user}, "mobile_no"
            )
        ...
        qry = """select `tabContact`.name 'contact_name' from `tabDynamic Link`
				...
				where `tabDynamic Link`.link_doctype='Customer' and link_name='{0}'""".format(
            customer.name
        )
...
def update_in_cart(
    ...
    from ecs_vim.api import update_cart

    slot_name = frappe.get_value(
        "Item", {"item_code": item_code}, ["non_sharable_slot"], as_dict=True
    ).non_sharable_slot
    if slot_name == 0:
        slot = None
        uom = None
...
def place_order(advance_amount=0, payment_status="", transactionid=""):
    from ecs_vim.api import place_order as vim_place_order

    return vim_place_order(
        restapi=True,
...
def check_internal_user():
    from ecs_vim.api import is_internal_user
...
def get_customer_name(phone):
    from ecs_vim.api import get_customer
...
def apply_coupon_code(applied_code):
    applied_referral_sales_partner = (
        False  # If needed in future had to take it as input
    )
    quotation = True
    ...
    def validate_coupon_code(coupon_name):
        ...
        if coupon.valid_from:
            if coupon.valid_from > getdate(today()):
                return {
                    "error": True,
                    "message": _("Sorry, this coupon code's validity has not started"),
                }
        if coupon.valid_upto is not None:
            if coupon.valid_upto < getdate(today()):
                return {
                    "error": True,
                    "message": _("Sorry, this coupon code's validity has expired"),
                }
        if coupon.used >= coupon.maximum_use:
            return {
                "error": True,
                "message": _("Sorry, this coupon code is no longer valid"),
            }
...
def get_unit_wise_price(item_code=None, uom=None):
    from ecs_vim.api import get_price
...
def update_data(**kwargs):
    ...
    preferred_communication = (
        kwargs["preferred_communication"] if "preferred_communication" in kwargs else ""
    )
    child_family_details = (
        kwargs["child_family_details"] if "child_family_details" in kwargs else ""
    )
    adult_family_details = (
        kwargs["adult_family_details"] if "adult_family_details" in kwargs else ""
    )
    customer_family_detail = (
        kwargs["customer_family_detail"] if "customer_family_detail" in kwargs else ""
    )
    ...
    if mobile_error_throw:
        return {
            "error": True,
            "message": "Mobile Number should be of length 10, consists of only numbers 0-9 and start with 0 ",
            "mobile_no": mobile_no,
        }
    ...
    query = """delete from `tabCustomer Family Detail` where parent='{0}'""".format(
        name
    )
    ...
    if doc:
        ...
        doc.favourite_venue = (
            kwargs["favourite_venue"] if "favourite_venue" in kwargs else ""
        )
        arr_child = []
        if customer_family_detail:
            ...
            for j in arr_child:
				...
				and child_name='{1}' and dob='{2}' and name !='{3}'""".format(
                    name, j.get("child_name", ""), j.get("dob"), ""
                )
                data = frappe.db.sql(query, as_dict=True)
                if data:
                    return {
                        "error": True,
                        "message": "Duplicate Family Details Found For Relation ("
                        + str(j.get("relation"))
                        + ")",
                    }
                ...
                if j.get("gender"):
                    dftl.gender = j.get("gender", "")
                if j.get("favourite_colour"):
                    dftl.favourite_colour = j.get("favourite_colour", "")
        ...
        if customer_family_detail:
            ...
            for j in arr_adult:
                ...
                if not j.get("first_name") or not j.get("last_name"):
                    return {
                        "error": True,
                        "message": "Enter First Name & Last name in Family Details for ("
                        + j.get("relation")
                        + ")",
                    }
                query = """select * from `tabCustomer Family Detail` where parent='{0}' 
				and person_name='{1}' and dob='{2}' and name !='{3}'""".format(
                    name, personname, j.get("dob"), ""
                )
                data = frappe.db.sql(query, as_dict=True)
                if data:
                    return {
        ...
        if perc == 100 and not doc.first_coupon:
            ...
            send_first_coupon_mail(
                subject="Free Coupon for profile completion", coupon=coupon_code
            )
            # doc.first_coupon == 1
            # doc.save()
    return {
        "updated_data": profile_data,
        "profile_percetage": perc,
...
def profile_percetage(profile_data, arr_adult, arr_child):
    ...
    if (
        ...
        and profile_data.get("last_name")
        and profile_data.get("email_id")
        and profile_data.get("preferred_communication")
        and profile_data.get("nationality")
        and profile_data.get("city")
    ):
        percent += 50
...
def get_customerdata():
    ...
    if user:
        ...
        for d in frappe.get_all(
            "Contact", fields=("name"), or_filters={"email_id": user, "user": user}
        ):
            contact_name = frappe.db.get_value("Contact", d.name)
            if contact_name:
                contact = frappe.get_doc("Contact", contact_name)
                ...
                if not contact.email_id:
                    ...
                    for email_row in contact.email_ids:
                        if (
                            ...
                            and email_row.email_id != frappe.session.user
                        ):
                            email_row.is_primary = 0
                        if (
                            not email_row.is_primary
                            and email_row.email_id == frappe.session.user
                        ):
...
def create_coupon(customer):
    ...
    coupon = frappe.get_doc(
        {
            ...
            "pricing_rule": frappe.db.get_value(
                "Selling Settings", None, "coupon_code_default_pricing_rule"
            ),
    ...
    coupon.save()
    frappe.db.sql(
        """update `tabCustomer` set first_coupon = '1' where name = "{}" """.format(
            customer
        )
    )
    frappe.db.commit()
...
def getSocialLogins():
    ...
    for provider in providers:
        ...
        icon = get_icon_html(
            frappe.get_value("Social Login Key", provider, "icon"), small=True
        )
...
def get_oauth2_providers():
    ...
    for provider in providers:
        ...
        if provider.api_endpoint_args:
            out[provider.name]["api_endpoint_args"] = json.loads(
                provider.api_endpoint_args
            )
...
def get_oauth2_authorize_url(provider, redirect_to):
    ...
    frappe.cache().set_value(
        "{0}:{1}".format(provider, state["token"]), True, expires_in_sec=120
    )
...
def login_via_oauth2(provider, code, state, decoder=None):
    ...
    if not info:
        {"error": ("Email not verified with {0}").format(provider.title())}
...
def login_oauth_user(
    ...
    if not (state and state["token"]):
        return {
            "error": _("Invalid Request. Token is missing"),
            "http_status_code": 417,
        }

    token = frappe.cache().get_value(
        "{0}:{1}".format(provider, state["token"]), expires=True
    )
    if not token:
    ...
    if not user:
        return {
            "error": _(
                "Invalid Request. Please ensure that your profile has an email address"
            )
        }
    ...
    try:
        ...
        return {
            "Error": "Sorry. Signup from Website is disabled.",
            "success": False,
    ...
    if frappe.utils.cint(generate_login_token):
        ...
        frappe.cache().set_value(
            "login_token:{0}".format(login_token),
...
def update_oauth_user(user, data, provider):
    ...
    if provider == "facebook" and not user.get_social_login_userid(provider):
        ...
        user.update(
            {
                "user_image": "https://graph.facebook.com/{id}/picture".format(
                    id=data["id"]
                )
            }
        )
        ...
        save = True
        user_id_property = (
            frappe.db.get_value("Social Login Key", provider, "user_id_property")
            or "sub"
        )
