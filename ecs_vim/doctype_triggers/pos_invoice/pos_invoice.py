...
def on_submit(self, method):
    ...
    if self.applied_coupen:
        coupon_list = frappe.get_all(
            "Coupon Code", filters={"coupon_code": self.applied_coupen.upper()}
        )
        if not coupon_list:
            ...
            for itm in bom_doc.items:
                ...
                if itm and itm.reusable_item:
                    ...
                    if frappe.db.exists("Asset", {"item_code": itm.item_code}):
                        ...
                        asset.save()
                        if frappe.db.exists(
                            "Invoice Schedule", {"pos_invoice": self.name}
                        ):
                            invoice_schedule = frappe.get_doc(
                                "Invoice Schedule", {"pos_invoice": self.name}
                            )
                            ...
                            if journal_entry:
                                ...
                                je.cancel()
    event = "select name from `tabEvent Booking` where reference_name='POS Invoice' and reference_id='{0}'".format(
        self.name
    )
...
def make_depreciation_schedule(self, asset):
    ...
    depreciation_amount = flt(asset.gross_purchase_amount) / flt(
        asset.maximum_usage_count
    )
    ...
    for d in asset.get("invoice_schedule"):
        if not d.journal_entry and getdate(d.schedule_date) <= getdate(date):
            ...
            for dimension in accounting_dimensions:
                if asset.get(dimension["fieldname"]) or dimension.get(
                    "mandatory_for_bs"
                ):
                    credit_entry.update(
                        {
