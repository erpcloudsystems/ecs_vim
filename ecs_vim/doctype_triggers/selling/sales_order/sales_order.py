...
def before_submit(self, method):
    ...
    if self.delivery_date:
        ...
        delivery_date = (
            self.delivery_date.strftime("%m-%d-%Y")
            if isinstance(self.delivery_date, datetime.datetime)
            or isinstance(self.delivery_date, datetime.date)
            else self.delivery_date
        )
    ...
    if self.select_event:
        ...
        if item_doc:
            ...
            if nonsharable_slot == 1:
                if not self.brand or not self.branch or not self.city:
                    frappe.throw(
                        "Please select Branch, Brand, City Because you have select Non-Sharable-Slot Event"
                    )

        slot_list = "select start_time,end_time from `tabSlot List` where slot_name='{0}' and parent = '{1}'".format(
            self.select_slot, self.select_event
        )
        ...
        if slist:
            ...
            if nonsharable_slot == 1:
                ...
                doc.subject = (
                    self.select_event + "-" + new_date + "-" + self.select_slot
                )
            if self.select_event:
...
def on_cancel(self, method):
    event = "select name from `tabEvent Booking` where reference_name='Sales Order' and reference_id='{0}'".format(
        self.name
    )
...
def get_item_list(doctype, txt, searchfield, start, page_len, filters):
    ...
    unitdetails = frappe.db.sql(
		...
		limit %(start)s, %(page_len)s""",
        {
            ...
            "is_sales_item": filters.get("is_sales_item"),
...
def get_family_details(customer_name):
    family_details = (
        """select * from `tabCustomer Family Detail` where parent='{0}'""".format(
            customer_name
        )
    )
...
def get_slot_list(item_name, is_new, delivery_date):
    result_list = []
    unitdetail = (
        "select name from `tabItem` where  non_sharable_slot = 1 and name='{0}'".format(
            item_name
        )
    )
    ...
    if shareable_slot:
        if is_new:
            slot_list = "select slot_name from `tabSlot List` where parent='{0}' and  slot_name not in (select select_slot from `tabSales Order` where select_event='{0}' and docstatus !=2 and date_format(delivery_date,'%Y-%m-%d')='{1}') ".format(
                item_name, delivery_date
            )
        else:
            slot_list = (
                "select slot_name from `tabSlot List` where parent='{0}'".format(
                    item_name
                )
            )
...
def get_item_details(item_name):
    ...
    unitdetail = """select `tabItem`.item_code as 'item_code',`tabItem`.item_name as 'item_name',`tabItem`.stock_uom as 'stock_uom', `tabItem Price`.price_list_rate as 'rate',`tabItem`.non_sharable_slot 'non_sharable_slot' from `tabItem` 
					...
					where `tabItem`.item_code='{0}'""".format(
        item_name
    )
