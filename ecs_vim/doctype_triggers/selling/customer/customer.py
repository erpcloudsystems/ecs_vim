...
def validate(self, method):
    ...
    if mobile_error_throw:
        frappe.throw(
            "Mobile Number should be of length 10, consists of only numbers 0-9 and start with 0 "
        )
    ...
    if self.get("customer_family_detail"):
        for i in self.get("customer_family_detail"):
			...
			and person_name='{1}' and dob='{2}' and name !='{3}'""".format(
                self.name, i.person_name, i.dob, i.name
            )
            ...
            if data:
                frappe.throw(
                    "Duplicate Family Details Found For Row No (" + str(i.idx) + ")"
                )
...
def syncSalesforce(self):
    if self.salesforceid:
        ...
        if deleted_rec:
                ...
                post_data[0]["ContactsUpdate"].append(
                    {
                        ...
                        "Active": "No",
                        "FamilyRelationship": "Main"
                        if (fam.relation and fam.relation.lower()) == "self"
                        else fam.relation,
                    }
                )
            ...
            if post_data:
                get_config()
                res, sync_time, logid = sendSalesforceReq(
                    "/services/apexrest/api/UpdateFamilyAccount", post_data
                )
...
def reset_syncedflag(self, method):
    self.db_set(
        "salesforce_synced", False, notify=False, commit=True, update_modified=False
    )
