...
from erpnext.accounts.doctype.pos_profile.pos_profile import (
    get_item_groups as si_get_item_groups,
)
...
def get_pos_items(
    ...
    if items_data:
        ...
        item_bundles_data = frappe.get_all(
            "Product Bundle",
...
def item_group_query(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql(
        """ select distinct name from `tabItem Group`
			where {condition} and (name like %(txt)s) order by idx  limit {start}, {page_len}""".format(
...
def get_slot_list(item_name, is_new, delivery_date):
    ...
    slotdetail = """select slot_name from `tabSlot List` where parent='{0}'  and
	slot_name not in (select slot from `tabEvent Booking`  where  date='{1}'and event_item='{2}' )""".format(
        item_name, delivery_date, item_name
    )
...
def get_combo_items(item):
    ...
    res = {"items": result, "sets": cnt_result, "ditems": dresult}
...
def get_items(sorder):
    ...
    if items_data:
        ...
        for item in items_data:
            ...
            row = {}
            row.update(item)
            row.update({"bundle_item": item_bundle.get("name")})

            result_list.append(row)
        packed_item_dta = frappe.get_all(
            "Packed Item",
            fields=["parent_item", "item_code", "qty"],
            filters={
                "parent_item": ["in", items],
                "parent": sorder,
                "parenttype": "Sales Order",
            },
        )
        item_bundles_detail = frappe.get_all(
            "Product Bundle Item",
            fields=[
                "name",
                "parent",
    ...
    return {"result_list": result_list, "packed_list_data": packed_list_data}
...
def make_condition(item_code):
    ...
    condition += """item.name like {item_code}
		or item.item_name like {item_code}""".format(
    ...
    if search_fields:
        for field in search_fields:
            condition += " or item.{0} like {1}".format(
                field["fieldname"], frappe.db.escape("%" + item_code + "%")
            )
    return condition
...
def create_event_booking(
    pos_invoice, event, posting_date, slot, brand, city, branch, department=""
):
    import json
...
def update_POS(pos_invoice, brand, city, branch, slot, event, department=""):
    ...
    docitem = frappe.get_doc(
        "POS Invoice Item", {"parent": pos_invoice, "item_name": item_name}
    )
...
def update_accounting_dimension(sales_order, pos_invoice):
    ...
    for ds in docso.items:
        if ds.is_nonsharable_item:
            docitem = frappe.get_doc(
                "POS Invoice Item", {"parent": pos_invoice, "item_code": ds.item_code}
            )
            if ds.slot_name:
                docslot = frappe.get_doc(
                    "Slot List", {"slot_name": ds.slot_name, "parent": ds.item_code}
                )
...
def get_item_group(pos_profile):
    ...
    item_sub_groups = frappe.db.sql(
        """Select `ig`.`name`,`pig`.`idx`,`ig`.`parent_item_group`
		from `tabItem Group` as `ig`
		inner join `tabPOS Item Group` as `pig`
			on `ig`.`name` = `pig`.`item_group`
		where `pig`.`parent` = '{0}' and parent_item_group!='All Item Groups'
    ...
    return {"item_group_dict": item_group_dict, "item_sub_group_dict": item_sub_groups}
...
def validate_user_permission(appuser, apppassword):
    ...
    sales_manager_roles = frappe.db.sql(
        """select role_name from `tabRole`  where {cond}""".format(cond=cond), as_dict=1
    )
...
def check_user_password(
    ...
    username = frappe.db.sql(
        """select u.name from tabUser u where u.username='{0}'""".format(user)
    )

    auth = frappe.db.sql(
        """select `name`, `password` from `__Auth`
		where `doctype`=%(doctype)s and `name`=%(name)s and `fieldname`=%(fieldname)s and `encrypted`=0""",
        {"doctype": doctype, "name": username, "fieldname": fieldname},
        as_dict=True,
    )
...
def apply_pricing_rule(args, doc=None):
    ...
    for item in item_list:
        ...
        data = get_pricing_rule_for_item(
            args_copy, item.get("price_list_rate"), doc=doc
        )
...
def get_pricing_rule_for_item(args, price_list_rate=0, doc=None, for_validate=False):
    ...
    if pricing_rules:
        ...
        for pricing_rule in pricing_rules:
            ...
            coupon_code_list = frappe.db.get_list(
                "Coupon Code", {"pricing_rule": pricing_rule.name}, pluck="coupon_code"
            )
            pricing_rule_coupon = frappe.db.get_value(
                "Coupon Code", {"coupon_code": coupon_code}, ["pricing_rule"]
            )
            if (
                doc.couponcode.upper() == coupon_code.upper()
                or doc.couponcode.upper() in coupon_code_list
                or doc.couponcode in coupon_code_list
            ):
                ...
                if pricing_rule.mixed_conditions or pricing_rule.apply_rule_on_other:
                    item_details.update(
                        {
                            "apply_rule_on_other_items": json.dumps(
                ...
                if (
                    ...
                    and pricing_rule.name == pricing_rule_coupon
                ):
                    if not pricing_rule.validate_applied_rule:
                        if pricing_rule.price_or_product_discount == "Price":
                            ...
                            get_product_discount_rule(
                                pricing_rule, item_details, args, doc
                            )
...
def apply_coupon_code(
    doc, applied_code=None, applied_referral_sales_partner=None, warehouse=None
):
    quotation = True
    doc = json.loads(doc)
    ...
    coupon_list = frappe.get_all(
        "Coupon Code", filters={"coupon_code": applied_code.upper()}
    )
    if not coupon_list:
        frappe.throw(_("Please enter a valid coupon code"))
    coupon_name = coupon_list[0].name
    ...
    if coupendoc.customer:
        if doc.customer != coupendoc.customer:
            frappe.throw(
                _(
                    """Coupon not valid for the customer  '{0}'...,Please enter a valid coupon code""".format(
                        doc.customer
                    )
                )
            )
    ...
    item_list = frappe.db.sql(
        """select * from `tabPricing Rule Item Code` where parent ='{0}'""".format(
            pricing_rule_doc.name
        ),
        as_dict=True,
    )
...
def validate_coupen_item(items, applied_code):
    ...
    if quotation:
        ...
        if (
            pricing_rule_doc.warehouse
            and brdoc.default_warehouse != pricing_rule_doc.warehouse
        ):
            frappe.throw(
                _(
                    " Coupon not valid for warehouse '{0}'".format(
                        brdoc.default_warehouse
                    )
                )
            )
