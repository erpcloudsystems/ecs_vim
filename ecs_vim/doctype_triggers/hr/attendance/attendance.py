...
def validate(doc, method):
	if validate_ss_processed(doc.employee,doc.attendance_date) : frappe.throw("Can not create or edit attendance, salary is already processed for this month")
	if not doc.is_new():
...
def on_trash(doc, method):
	if validate_ss_processed(doc.employee,doc.attendance_date) : frappe.throw("Can not delete attendance, salary is already processed for this month")
...
def process_attendance(doc, ot_request=0,overtime_without_punch= 0):
	...
	weak_off(doc)
	get_ot(doc=doc, ot_request=ot_request, present=present,overtime_without_punch = overtime_without_punch)
	short_time(doc)
...
def on_cancel(doc, method):
	...
	if validate_ss_processed(doc.employee,doc.attendance_date) : frappe.throw("Can not cancel attendance, salary is already processed for this month")
	...
	if doc.get('_cancel_flag', 1) and doc.get('amended_request'):
		...
		if amend_doc.name:
			frappe.db.sql("""update `tabAttendance Amendment` set `attendance`= NULL where name='{0}'""".format(amend_doc.name))
			frappe.db.commit()
			frappe.db.sql("""update `tabAttendance` set `amended_request`= NULL where name='{0}'""".format(doc.name))
			frappe.db.commit()
			amend_doc.cancel()
	...
	emp_checkin_data = frappe.db.get_list('Employee Checkin', filters={ 'attendance': doc.name }, fields=['name'])
	if emp_checkin_data:
...
def validate_leave_applications(doc,method):
	if doc.get("leave_application"):
		leave_application = frappe.get_doc('Leave Application', doc.get("leave_application"))
...
def get_ot(doc, ot_request=0, present=0,overtime_without_punch= 0):
	...
	ot_planner = """SELECT approved_maximum_ot_allowed_per_day, 0 AS default_ot,0 AS apply_overtime_without_any_attendance_punches
		...
		AND status = 'Approved'
		AND applicant = '{0}'
		AND ot_request_date = '{1}'""".format(doc.get('employee'), doc.get('attendance_date'))
	...
	default_ot = 0
	if ot_planner and (present or overtime_without_punch or (ot_planner[-1] and ot_planner[-1]["apply_overtime_without_any_attendance_punches"]) or 0):
		ot_dict = ot_planner[-1]
		if ot_dict.get('default_ot'):
			default_ot = ot_dict['approved_maximum_ot_allowed_per_day']
	
	attendance_details = ot_worked(employee=doc.get('employee'), attendance_date=getdate(doc.get('attendance_date')),doc=doc)
	
	ot_hours = attendance_details.get('ot_hours') or default_ot
	if overtime_without_punch or (ot_planner and ot_planner[-1]["apply_overtime_without_any_attendance_punches"]): 
		ot_hours = default_ot
	ot_rule = applied_ot_rule(employee=doc.get('employee'), to_date=getdate(doc.get('attendance_date')))
	...
	if ot_hrs_min < (ot_rule_doc.get('minimum_ot_limit') or 0):
		ot_hrs_min = 0
	ot_hrs_min = (abs(ot_hrs_min / (ot_rule_doc.get('ot_slab') or 1)) * (ot_rule_doc.get('ot_slab') or 1))
	...
	if ot_hrs_min > 0:
		if not ot_planner:
			...
			if ot_hrs_min > (ot_planner.get('approved_maximum_ot_allowed_per_day') or 0):
				if ot_planner.get('violation_action') == 'Violation approval required':
					# create violation
					return
				else:
					if (ot_planner.get('approved_maximum_ot_allowed_per_day') or 0) != 0:
						ot_hrs_min = (ot_planner.get('approved_maximum_ot_allowed_per_day') or 0)
		import math
		doc.ot_hours_minutes = flt(math.ceil(ot_hrs_min), 0)
		doc.ot_hours = flt(round(ot_hrs_min) / 60,2)
		doc.ot_rule = ot_rule
		
		if doc.get('status') in ('Absent', 'On Leave') and (doc.get('ot_hours_minutes') or 0) and ( attendance_details.get("in_time") and  attendance_details.get("out_time")) :
		...
		if doc.get('working_hours') and doc.get('working_hours') < round(attendance_details.get("total_working_hours"),2) :
			doc.working_hours = round(attendance_details.get("total_working_hours"),2)
		if not attendance_details.get("holiday_weekoff") and attendance_details.get("total_working_hours") and doc.status == "Present" and (overtime_without_punch or (ot_planner and ot_planner["apply_overtime_without_any_attendance_punches"] and attendance_details.get("total_working_hours")>= doc.working_hours)) :
...
def ot_worked(employee, attendance_date, error=False,doc=None):
	current_shift = get_employee_shift(employee=employee, for_date=attendance_date, consider_default_shift=True, next_shift_direction='forward')
	...
	if current_shift:
		...
		logs = """SELECT *
			...
			AND shift_end = '{}'
			AND shift = '{}'
			ORDER BY employee, time""".format(employee, current_shift.start_datetime, current_shift.end_datetime, current_shift.shift_type.name)
		...
		if logs or (doc and doc.amended_request):
			if doc and doc.get('amended_request'):
					amend_req = """SELECT *
							...
							WHERE name = '{}'""".format(doc.get('amended_request'))
					amend_req = frappe.db.sql(amend_req, as_dict=True)
		
					if amend_req:
						amend_req = amend_req[0]
						first_in_punch = get_datetime('{} {}'.format(doc.get('attendance_date'), amend_req.get('amended_in_time')))
						last_out_punch = get_datetime('{} {}'.format(doc.get('attendance_date'), amend_req.get('amended_out_time')))
						
						total_working_hours = time_diff_in_hours(first_in_punch.replace(second=0, microsecond=0), last_out_punch.replace(second=0, microsecond=0))
						in_time=first_in_punch
						out_time=last_out_punch
			else:
					total_working_hours, in_time, out_time = custom_calculate_working_hours(logs, current_shift.shift_type.determine_check_in_and_check_out, current_shift.shift_type.working_hours_calculation_based_on)
			...
			if not holiday_weekoff :
				attendance = """SELECT *
					...
					WHERE docstatus = 1
					AND employee = '{}'
					AND attendance_date ='{}'""".format(employee, attendance_date)
				...
				if attendance and attendance [0]['name']:
					...
					if att_doc.leave_type :  holiday_weekoff = True
			
			if holiday_weekoff:
				if in_time and out_time:
					total_working_hours = (out_time.replace(second=0, microsecond=0) - in_time.replace(second=0, microsecond=0)).total_seconds() / 3600
				total_ot = total_working_hours
				
				ot_start_time = get_datetime(in_time)
			else:
				att_intime = get_datetime(in_time or current_shift.start_datetime).replace(second=0, microsecond=0)
				shift_intime = get_datetime(current_shift.start_datetime).replace(second=0, microsecond=0)
				early_coming_ot = (shift_intime - att_intime).total_seconds()
				early_coming = ot_rule_doc.get('include_early_coming_ot') if not ot_rule_doc.get('include_early_coming_ot') is None else 1
				early_coming_ot = early_coming_ot if early_coming else 0

				att_outtime = get_datetime(out_time or current_shift.end_datetime).replace(second=0, microsecond=0)
				shift_outime = get_datetime(current_shift.end_datetime).replace(second=0, microsecond=0)
			...
			frappe.msgprint(_("Employee Checkin Not Found for Employee {} within Shift {} Between {} and {}".format(employee, current_shift.shift_type.name, current_shift.start_datetime, current_shift.end_datetime)))
	elif error:
		frappe.msgprint(_("Shift Not Found for Employee {} in Date {}".format(employee, formatdate(attendance_date))))
...
def custom_calculate_working_hours(logs, check_in_out_type, working_hours_calc_type):
	...
	if check_in_out_type == 'Alternating entries as IN and OUT during the same shift':
		...
		if working_hours_calc_type == 'First Check-in and Last Check-out':
			...
			total_hours = time_diff_in_hours(in_time.replace(second=0, microsecond=0), logs[-1].time.replace(second=0, microsecond=0))
		elif working_hours_calc_type == 'Every Valid Check-in and Check-out':
			logs = logs[:]
			while len(logs) >= 2:
				total_hours += time_diff_in_hours(logs[0].time.replace(second=0, microsecond=0), logs[1].time.replace(second=0, microsecond=0))
				del logs[:2]
		...
		if working_hours_calc_type == 'First Check-in and Last Check-out':
			...
			if first_in_log and last_out_log:
				...
				total_hours = time_diff_in_hours(in_time.replace(second=0, microsecond=0), out_time.replace(second=0, microsecond=0))
		elif working_hours_calc_type == 'Every Valid Check-in and Check-out':
			in_log = out_log = None
			for log in logs:
				if in_log and out_log:
					...
					total_hours += time_diff_in_hours(in_log.time.replace(second=0, microsecond=0), out_log.time.replace(second=0, microsecond=0))
					in_log = out_log = None
				if not in_log:
...
def weekoff_holiday(employee, date):
	current_shift = get_employee_shift(employee=employee, for_date=getdate(date), consider_default_shift=True, next_shift_direction='forward')
	...
	if current_shift:
		weak_off = """SELECT `H`.`is_weekoff`
			...
			""".format(employee, date, current_shift.shift_type.get('name'))
		return frappe.db.sql(weak_off, as_dict=True), current_shift.shift_type.get('name')
	else:
		return [], None


def weak_off(doc):
	if doc.get('status') == 'On Leave': return
	is_weekoff_shift = frappe.db.get_value('Shift Type', doc.get('shift'), 'is_weekoff')
	weak_off, _shift = weekoff_holiday(doc.get('employee'), getdate(doc.get('attendance_date')))
	doc.leave_type = (get_leve_name('WO') if is_weekoff_shift or(weak_off[0] and weak_off[0]['is_weekoff']) else get_leve_name('H')) if weak_off or is_weekoff_shift else None
...
def short_time(doc):
	...
	if doc.get('status') != 'Present' or not doc.get('shift'): return
	
	shift = """SELECT *
		FROM `tabShift Type`
		WHERE name = '{}'""".format(doc.get('shift'))
	shift = frappe.db.sql(shift, as_dict=True)
	shift = shift[0] if shift else {}

	shift_start_time = get_datetime('{} {}'.format(doc.get('attendance_date'), (shift.get('start_time') or '00:00:00')))

	end_date = doc.get('attendance_date')
	if shift.get('start_time') >= shift.get('end_time'):
		end_date = add_days(end_date, 1)

	shift_end_time = get_datetime('{} {}'.format(end_date, (shift.get('end_time') or '00:00:00')))
	
	first_in_punch = get_datetime(doc.get('attendance_in_punch'))
	if not doc.get('attendance_in_punch'):
		first_in_punch = get_datetime('{} {}'.format(doc.get('attendance_date'), '00:00:00'))

	last_out_punch = get_datetime(doc.get('attendance_out_punch'))
	if not doc.get('attendance_out_punch'):
		last_out_punch = get_datetime('{} {}'.format(doc.get('attendance_date'), '00:00:00'))
	...
	if doc.get('amended_request'):
		...
		att_punch = """SELECT *
			...
			and `EC`.`shift_end` = '{3}'
			ORDER BY `EC`.`time` ASC""".format(doc.get('employee'), doc.get('shift'), shift_start_time, shift_end_time)
	...
	if (doc.get('status') == 'Present' and doc.get('leave_type')): return

	shift['start_datetime'] = shift_start_time
	shift['end_datetime'] = shift_end_time

	skip_late_comming = frappe.db.get_value('Employee', doc.get('employee'), 'not_to_account_late_coming')

	doc.late_coming_minutes = 0 if skip_late_comming else late_coming(shift, first_in_punch) 
	doc.early_going_minutes = 0 if skip_late_comming else early_going(shift, last_out_punch)
...
def add_ot_in_earlygoing_hrs(doc):
	current_shift = get_employee_shift(employee=doc.get('employee'), for_date=getdate(doc.get('attendance_date')), consider_default_shift=True, next_shift_direction='forward')
	shift_outime = get_datetime(current_shift.end_datetime).replace(second=0, microsecond=0)
	if get_datetime(doc.attendance_out_punch) > shift_outime:
		doc.working_hours = (shift_outime - get_datetime(doc.attendance_in_punch) ).total_seconds() / 3600

	doc.working_hours = (doc.working_hours or 0 ) + (int(doc.get('ot_hours_minutes'))or 0)/60

def late_coming(shift, time):
	if not time or not shift: return 0
...
def make_present(doc, present=0):
	...
	if doc.get('status') == 'Present': return
	...
	if current_shift and current_shift.shift_type :
		is_weekoff_shift = frappe.db.get_value('Shift Type', current_shift.shift_type.get('name'), 'is_weekoff')
	if is_weekoff_shift :
		doc.shift = current_shift.shift_type.get('name')
		weak_off(doc)
	if current_shift and (shift_up or (doc.get('status') in ('Absent', 'On Leave') and (doc.get('ot_hours_minutes') or 0))):
		doc.shift = current_shift.shift_type.get('name')
		if current_shift and shift_up :
			doc.attendance_in_punch = get_datetime(current_shift.start_datetime)
			doc.attendance_out_punch = get_datetime(current_shift.end_datetime) + timedelta(minutes=int(doc.get('ot_hours_minutes') or 0))
			
			doc.working_hours = (get_datetime(doc.attendance_out_punch) - get_datetime(doc.attendance_in_punch)).total_seconds() / 3600
			doc.custom_in_time = get_datetime(doc.attendance_in_punch).time()
			doc.custom_out_time = get_datetime(doc.attendance_out_punch).time()
		...
		doc.shift = None
		doc.attendance_in_punch = get_datetime('{} {}'.format(doc.get('attendance_date'), '00:00:00'))
		doc.attendance_out_punch = get_datetime('{} {}'.format(doc.get('attendance_date'), '00:00:00'))
		doc.working_hours = 0
		doc.custom_in_time = '00:00:00'
		doc.custom_out_time = '00:00:00'
...
def make_absent(doc):
	...
	logs = """SELECT *
		...
		AND shift_end = '{}'
		AND shift = '{}'
		ORDER BY employee, time""".format(doc.get('employee'), current_shift.start_datetime, current_shift.end_datetime, current_shift.shift_type.name)
	logs = frappe.db.sql(logs, as_dict=True)
